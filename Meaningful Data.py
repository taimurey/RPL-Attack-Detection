...
while counter < len(np_Raw_Data):
    ...
    if counter != 0 and counter + 1 < len(np_Raw_Data):
        duration = np.float32(
            np_Raw_Data[counter][0])-np.float32(np_Raw_Data[counter - 1][0])
...
destination_unique_array = np.unique(
    np.array(Raw_Data.iloc[:, 2:3].astype(str)))
...
all_ip_addresses = np.concatenate(
    (source_unique_array, destination_unique_array))
...
row = pd.DataFrame(columns=['second', 'src', 'dst', 'packetcount', 'src_ratio', 'dst_ratio', 'src_duration_ratio', 'dst_duration_ratio',
                   'TotalPacketDuration', 'TotalPacketLenght', 'src_packet_ratio', 'dst_packet_ratio', 'DioCount', 'DisCount', 'DaoCount', 'OtherMsg', 'label'])

while counter < duration:

    # one_second_frame variable holds rows for 1 second duration.
    one_second_frame = np_Raw_Data[np.where(np.logical_and(
        np_Raw_Data[:, 0] >= currentSecond, np_Raw_Data[:, 0] <= currentSecond + 1.0))]
    ...
    if one_second_frame.size > 1:
        ...
        for i in packetcount:
            ...
            # calculating the source duration ratio in 1 second frame
            src_packet_ratio = src_packet_lenght_sum[sourcee] / \
                frame_packet_length_sum

            # calculating the destination duration ratio in 1 second frame
            dst_packet_ratio = dst_packet_lenght_sum[destinatt] / \
                frame_packet_length_sum
